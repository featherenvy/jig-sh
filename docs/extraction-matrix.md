...
| `.agent/jig-contract.json` | Templated | Declares the command-backed repo contract for CLI and MCP consumers, with SQLx tools gated by `sqlx_enabled`. |
...
| `scripts/check-agent-guides.sh` | Extracted + simplified | Kept structural checks; removed source-specific wording rules. |
