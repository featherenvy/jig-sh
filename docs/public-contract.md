...
Vault JSON is runtime-owned same-version behavior, not generated contract schema. `vault status` currently reports both `exists` and `vault_file_exists`; both mean the encrypted `vault.json` file exists, not that the vault home directory exists. `vault run` returns buffered, redacted, lossy UTF-8 `stdout` and `stderr` strings plus raw process status fields; automation should use `result.exit_signal` to distinguish signal termination when that field is present, and otherwise branch on `result.exit_status`.
