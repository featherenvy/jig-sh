...
Bootstrap command JSON is also runtime-owned. `scripts/jig init` and `scripts/jig adopt` include an `adoption_report` object that summarizes created, modified, unchanged, managed-block, and todo items for human review; `scripts/jig update` instead includes a `render_report` for the managed-file refresh it just applied. Automation should treat those reports as same-version diagnostics, not as `.agent/jig-contract.json` response schemas.
